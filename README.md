...
빈칸(`—`)이면 기준 조건 그대로다. 앞은 학습·PEFT 하이퍼파라미터(rank·학습률·에폭),
뒤는 데이터·프롬프트 조건(규칙서·negative·downsample)이다.

### 표에서 읽히는 것 셋

**prerun에 `교사일치 29.7%`가 22줄이다.** 37건 중 11건이라 **"전부 negative"의 이론값**
이다. 같은 줄의 AM이 100.0인 것이 많은데, **형식은 완벽한데 판정을 한 종류만 냈다**는
뜻이다. 학습법이 달라도 **똑같은 방식으로** 무너졌다.

**`negative제거` 세 줄이 두 라운드 다 붕괴인데 교사일치가 59~70%다.** negative를 아예
빼면 반대쪽(전부 positive)으로 무너지는데, 홀드아웃에 positive가 많아 점수는 높게
나온다. **`됨` 기준을 통과할 뻔한 실패라 위험한 줄이다.**
