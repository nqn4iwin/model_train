...
def write_table(results: dict, stem: str = "sweep", holdout: str | None = None) -> list[str]:
    ...
    lines = [f"# 스윕 결과 -- {ruler_name(ROOT / (holdout or HOLDOUT))} 자", "",
             ...
             " `docs/베이스라인_기록.md` 참고.", "",
             "**판정이 한 종류뿐이면 `붕괴`다.**"
             " **2026-08-27까지는 여기에 더해 '무조건 negative에 빈 배열'이"
             " AM 만점을 받았다** -- `labels`가 비면 AM2·AM3이 검사할 것이 없어"
             " 자동 1점이었기 때문이다. 그날 AM8s와 같은 원리를 두 칸에도 적용해"
             " (검사할 것이 없으면 만점을 주지 않는다) 그 길을 막았다."
             " **그러므로 이 표의 AM 값은 그 이전 표와 나란히 놓으면 안 된다.**", "",
             ...
             " 올라 보이므로 두 열을 같이 본다.", "",
             "**`건진판정`에도 `교사일치`와 같은 바닥이 있다.** 위에 적은"
             " '무조건 한쪽' 값이 그대로 이 열의 바닥이다 -- 건져서 세든 파싱해서"
             " 세든 판정 한 칸을 교사와 맞춰 보는 것은 같기 때문이다."
             " **바닥에 앉은 줄이 실제로 나왔으므로**(교사일치 17%대에 쏠림 99%)"
             " 이 열이 90%대라고 배웠다는 뜻이 아니다. 판정 분포를 같이 본다.", "",
